...
管理端系統日誌下方的「進階」卡片現在分成三個獨立且預設收合的功能：既有歌曲歌詞轉檔、既有影片效能最佳化，以及既有影片 AI 去人聲。影片效能最佳化可將 MP4 轉成 H.264、最高 1080p；AI 去人聲會使用 Spleeter 產生原唱左聲道與伴奏右聲道；兩者成功後都會取代 `ktv_songs` 中的同名檔案。三個工具互不共用輸入欄位，且 AI 去人聲與其他製作工作不能同時執行。
