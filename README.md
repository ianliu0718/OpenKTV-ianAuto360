...
3.  **合成階段**：利用 FFmpeg 將三組立體聲合併為單一六聲道 AAC 音訊流，再與原畫質影像封裝為 `.mp4`。聲道 1-2 為原聲、3-4 為導唱（正常伴奏加小聲人聲）、5-6 為伴奏。導唱與伴奏可套用 `loudnorm`，原聲保留原始音訊。
...
管理後台的「批量新增 KTV 歌曲」可在文字區每行輸入一首歌曲，格式為 `YouTube 網址 | 歌名`。送出後由伺服器沿用單曲製作流程逐首執行，不會同時啟動多個 Spleeter 工作；單首失敗會記錄日誌並繼續下一首，最後統計成功與失敗數量。批量處理期間系統會維持忙碌鎖，避免其他下載或更新工作與 AI 分離同時執行。
...
管理端系統日誌下方的「進階」卡片現在分成四個獨立且預設收合的功能：既有歌曲歌詞轉檔、既有影片效能最佳化、既有影片 AI 去人聲，以及既有歌曲音訊平衡。影片效能最佳化會以 FFmpeg 重新編碼為 H.264、最高 1080p，系統日誌會依序顯示接收檔案、FFmpeg 執行、輸出檔大小、取代原 MP4 與最終成功或失敗結果；成功條件包含 FFmpeg 回傳碼正常、輸出檔案存在且檔案大小大於 0，成功後才取代原 MP4。Windows 會以 UTF-8 並容錯讀取 FFmpeg 輸出，避免系統 cp950 解碼例外。音訊平衡只重新編碼音訊並套用 `-16 LUFS`，影片畫面直接複製；四個工具互不共用輸入欄位，且高負載處理工作不能同時執行。

新增 KTV 歌曲時可勾選「製作完成後自動平衡歌曲音量」，系統會在合成音訊時套用 FFmpeg `loudnorm`，以 `-16 LUFS` 與 `-1.5 dBTP` 作為固定目標，減少不同歌曲之間的音量落差。這個處理發生在製作階段，不會增加播放時的 CPU 負擔。
