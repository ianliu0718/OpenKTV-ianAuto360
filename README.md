...
3.  **合成階段**：利用 FFmpeg 將原唱（左聲道）、伴奏（右聲道）與原畫質影像重新封裝為 `.mp4`，預設使用 `loudnorm` 固定整體響度，也可在管理端取消。
...
後台的「新增 KTV 歌曲」表單可選擇 `.srt`、`.lrc` 或 `.vtt` 字幕檔，也可在進階文字框直接貼上這三種格式；系統會依檔案副檔名或文字內容自動判斷格式，並在歌曲製作完成後轉換字幕，以歌曲 basename 儲存為同名 `.vtt`。系統日誌下方另設有預設收合的進階「既有歌曲歌詞轉檔」卡片，使用 `.mp4` 檔案選擇器指定既有歌曲，字幕輸出會儲存在歌曲相同位置並覆寫同名 VTT。`/player` 與 `/combo` 使用 HTML5 `<track>` 載入 WebVTT cue，再由播放畫面的自訂 KTV 渲染器顯示字幕：最多顯示兩行並以畫面中央為對齊基準，第一行位於中央左側且文字右對齊，第二行位於中央右側且文字左對齊；只有一行時自動置中，並固定在影片接近底部的位置。每個字會帶有短暫放大彈跳動畫，但不使用卡拉 OK 變色或填色效果。遙控器與一體機的「播放中」控制區可切換字幕顯示，並同步設定小字 80%、標準 100%、大字 120% 三種字幕大小；伺服器會將設定廣播至所有播放畫面，並同步更新控制端的目前選取狀態。字幕關閉時會清除自訂渲染內容並停用顯示。
...
管理端系統日誌下方的「進階」卡片現在分成四個獨立且預設收合的功能：既有歌曲歌詞轉檔、既有影片效能最佳化、既有影片 AI 去人聲，以及既有歌曲音訊平衡。音訊平衡只重新編碼音訊並套用 `-16 LUFS`，影片畫面直接複製；四個工具互不共用輸入欄位，且高負載處理工作不能同時執行。
