...
播放端與一體機播放畫面預設在右下角顯示遙控器 QR Code，手機掃描後會開啟同一台主機的 `/remote`；遙控器頁面可使用「顯示播放端 QR Code」開關，同步控制所有播放端與一體機的 QR Code 顯示狀態。開始播放歌曲時，兩個播放畫面會在中央顯示不含 `.mp4` 附檔名的歌曲名稱約 10 秒，之後自動淡出。
