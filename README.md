...
後台的「新增 KTV 歌曲」流程只輸入 YouTube 網址與歌名，不會自動產生字幕檔，也不會在歌曲完成後建立同名 VTT。若要加入字幕，請使用系統日誌下方預設收合的進階「既有歌曲歌詞轉檔」卡片，選擇既有 `.mp4` 與 `.srt`、`.lrc` 或 `.vtt` 字幕檔，或直接貼上字幕文字；系統會依檔案副檔名或文字內容自動判斷格式並儲存同名 VTT。`/player` 與 `/combo` 使用 HTML5 `<track>` 載入 WebVTT cue，再由播放畫面的自訂 KTV 渲染器顯示字幕：最多顯示兩行並以畫面中央為對齊基準，第一行位於中央左側且文字右對齊，第二行位於中央右側且文字左對齊；只有一行時自動置中，並固定在影片接近底部的位置。每個字會帶有短暫放大彈跳動畫，但不使用卡拉 OK 變色或填色效果。遙控器與一體機的「播放中」控制區可切換字幕顯示，並同步設定小字 80%、標準 100%、大字 120% 三種字幕大小；伺服器會將設定廣播至所有播放畫面，並同步更新控制端的目前選取狀態。字幕關閉時會清除自訂渲染內容並停用顯示。
