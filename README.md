...
3.  **合成階段**：利用 FFmpeg 將三組立體聲合併為單一六聲道 AAC 音訊流，再與原畫質影像封裝為 `.mp4`。聲道 1-2 為原聲、3-4 為導唱（正常伴奏加小聲人聲）、5-6 為伴奏。啟用音量平衡時，原聲、導唱與伴奏會分別套用 `loudnorm`，以 `-14 LUFS` 與 `-1 dBTP` 為共同基準。
...
管理後台的「批量新增 KTV 歌曲」可在文字區每行輸入一首歌曲，格式為 `YouTube 網址 | 歌名`。送出後由伺服器沿用單曲製作流程逐首執行，並共用「製作完成後自動平衡歌曲音量」勾選狀態；啟用時每首歌曲的原聲、導唱與伴奏都套用 `-14 LUFS / -1 dBTP`。批量處理不會同時啟動多個 Spleeter 工作；單首失敗會記錄日誌並繼續下一首，最後統計成功與失敗數量。批量處理期間系統會維持忙碌鎖，避免其他下載或更新工作與 AI 分離同時執行。
...
播放端與一體機的「音訊引擎」狀態列會在每首歌開始播放時顯示該 MP4 的整曲 Integrated Loudness（LUFS）。後端以 FFmpeg `ebur128` 分析第一條音訊流，並依歌曲檔案的修改時間與大小快取結果；這是診斷資訊，不會改變播放音量或 Web Audio 增益。數值越接近新增歌曲的 `-14 LUFS` 目標，代表整曲平均響度越接近標準化設定；若檔案不存在或分析失敗則只顯示音訊引擎名稱。
